---
title: "FET Bayesian Methods - HW3 - 7751512"
author: "Fabio Enrico Traverso"
date: "2022-12-08"
output: pdf_document
---
## Question 1  
```{r setup, echo=FALSE, message=FALSE, warning=FALSE, fig=TRUE}
...
```
Given the model structure, I expect that we cannot tell apart the effects of $\theta_2$ and $\theta_3$ on their joint distribution; hence, we cannot estimate the parameters $\theta_2,\theta_3$ consistently.The chunk above produces a long pdf because iterations get printed; I don't know how to fix that.
...
The model is a Multinomial, where we use the following distributional objects to obtain y_i: 
\begin{equation}
p(X_i|\beta,\sigma_{12},z_i), p(\beta|X_i,\sigma_{12},z_i), p(\sigma_{12}|X_i,\beta,z_i),p(z_j|X_i,\beta,\sigma_{12},y_i,z_{-j}), p(y_i|z_i)
\end{equation}
